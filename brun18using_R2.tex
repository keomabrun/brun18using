...
The result is that, if one frame isn't transmitted successfully (node $A$ does not receive a link-layer acknowledgement from node $B$), node $A$ retransmits \textit{on a different frequency}, thereby exploiting frequency diversity.
