...
A TSCH mode was added to the IEEE802.15.4 standard in its 2015 revision.
...
In a nutshell,
    time synchronization yield ultra low-power operation,
    channel yields ultra high reliability.
...
A new TSCH mode was added to the IEEE802.15.4 standard by the IEEE802.15.4e Task Group, to ``better support the industrial markets''.
...
A \smip network exhibits over 99.999\% end-to-end reliability\cite{watteyne09reliability}.
A \smip node (even when routing) consumes less than 50~$\mu A$ average current, resulting in over a decade of battery lifetime~\cite{watteyne10mitigating}.
